...
此外，从~\textbf{Android P}~（API Level 28）版本起，要求~App~进行网络请求时使用~https~协议进行加密网络请求，但天行数据API目前仍使用~http~协议
...
在~MainActivity~中定义了~okhttp3.Callback~接口对象，其~onResponse()~方法中首先通过~Response.body()~方法取得请求响应体对象并其
