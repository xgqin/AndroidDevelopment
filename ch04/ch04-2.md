...
initDb()方法的参数是**SQLiteDatabase**对象，在调用该方法时确保**SQLiteDatabase对象已经被初始化。
...
通过**SQLiteOpenHelper**类的**getReadableDatabase()**方法获得**SQLiteDatabase**对象，并通过该对象执行query()方法查询News表中的数据。
